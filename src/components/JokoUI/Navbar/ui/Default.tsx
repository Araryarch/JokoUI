...
const DefaultNavbar: React.FC<NavbarProps> = ({ title, content, links }) => {
  ...
  return (
    <>
      <nav
        className={`w-full h-16 bg-white text-black flex items-center justify-between px-6 fixed top-0 z-50 transition-all duration-300 ${
          isScrolled
      ...
      >
        ...
        <div className="hidden md:flex items-center gap-8">
          {links?.map((link, i) => (
            ...
            >
              {link.label}
              <span className="absolute bottom-[-4px] left-0 w-0 h-px bg-black transition-all duration-500 ease-out group-hover:w-full"></span>
            </a>
          ))}
        </div>
        ...
        <button
          className="md:hidden flex items-center justify-center w-10 h-10 rounded-full hover:bg-gray-100 transition-all duration-300 relative overflow-hidden"
      ...
      <div
        className={`fixed top-16 left-0 w-64 h-[calc(100vh-4rem)] bg-white text-black z-40 border-r border-gray-200 overflow-y-auto md:hidden`}
        style={{
        ...
        <div className="flex flex-col gap-6 p-6">
          {links?.map((link, i) => (
            <a
              key={link.href}
              href={link.href}
              className="text-lg font-semibold flex items-center gap-3 py-2 border-b border-gray-100 last:border-b-0 relative overflow-hidden group"
              onClick={toggleSidebar}
              style={{
            ...
            >
              <span className="absolute left-0 w-1 h-0 bg-black transition-all duration-300 group-hover:h-full"></span>
              {link.label}
            </a>
          ))}
          {content && (
